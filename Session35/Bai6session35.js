...
if (categorys) {
    ...
    categorys = [
        { categoryCode: "DM001", name: "Điện thoại", status: "statusOn" },
        { categoryCode: "DM002", name: "Đồ điện tử", status: "statusOff" }
    ];
}

let categoryIndex = -1;

function render(filteredCategorys) {
    tbody.innerHTML = "";
    filteredCategorys.forEach((element, index) => {
        ...
        edit.addEventListener("click", function() {
            categoryIndex = categorys.findIndex(el => el.categoryCode === element.categoryCode);
...
function filterCategorys() {
    ...
    if (selectedStatus !== "all") {
        filtered = categorys.filter(category => category.status === selectedStatus);
    }
...
statusFilter.addEventListener('change', filterCategorys);
